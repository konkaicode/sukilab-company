...
function App() {
  ...
  const [modeDurations, setModeDurations] = useState({ focus: 25 * 60, short: 5 * 60, long: 15 * 60 });
  const [secsLeft, setSecsLeft] = useState(modeDurations.focus);
  const [running, setRunning] = useState(false);
  const [completed, setCompleted] = useState(4);

  /* Stopwatch state */
  const [swSecs, setSwSecs] = useState(1832);
  ...
  { id: 2, title: "ランディングページのコピー作成", done: false, active: false },
  { id: 3, title: "Figmaレイアウトのレビュー", done: true, active: false },
  { id: 4, title: "チーム朝会のメモを準備", done: false, active: false }]
  );
